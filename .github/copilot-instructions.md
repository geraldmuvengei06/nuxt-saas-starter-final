...
UI & Styling: Tailwind CSS - Nuxt UI (utility-first)
...
Authentication is managed directly by the @nuxtjs/supabase module, which wraps Supabase's GoTrue-JS client.

Frontend auth actions (login, register, logout) are handled by calling methods on the supabase.auth object (e.g., supabase.auth.signInWithPassword()).

The module automatically handles session management via cookies and provides composables like useSupabaseUser() to access the user state globally.

A user's profile in our public profiles table must be created automatically upon successful registration. This should be handled by a Supabase Database Function (trigger).
