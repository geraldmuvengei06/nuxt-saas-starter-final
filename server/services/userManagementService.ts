...
export class UserManagementService {
  ...
  async bulkSyncUsers(
    event?: H3Event
  ): Promise<{ success: boolean; results: { synced: number; errors: number; details: string[] } }> {
    try {
    ...
    try {
      ...
      for (const user of users.users) {
        ...
        if (profile && profile.email !== user.email) {
          issues.syncIssues.push(
            `User ${user.id}: email mismatch (auth: ${user.email}, profile: ${profile.email})`
          )
        }
      }
  ...
  private async logUserActivity(
    ...
    try {
      const supabase = event ? await serverSupabaseClient(event) : await serverSupabaseServiceRole()
